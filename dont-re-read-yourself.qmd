...
1. How would you read in these ten files? Sketch it out.
2. What is different between the ten file paths? What is the same?
...
3. Are there any potential problems you can foresee?
...
Before you write any more of this, try asking `education_path()` for all ten years at once.
...
`read_csv()` will also tell you where each row came from, using `id` (similar to `bind_rows()`).
...
And `as.character()` turns it into text:

```{r}
#| label: step-character
basename(path) |> parse_number() |> as.character()
```

Why text? Because a year is a label, not a quantity. You are never going to want the mean of it.

Those three steps are one idea. Get the year out of a path. So let's give that idea a name:
