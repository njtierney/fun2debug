...
I really like to use `debugonce()` for this. Or, if you are inside `s` at the debug prompt inside that function, which steps you into that function.
...
The difference between `f` and `c` is the one worth having. `f` is "I have seen enough of this one", `c` is "I have seen enough of all of it".
...
5. **`n`, `s`, `f`, `c` and `Q`** are the controls worth knowing.
6. **The debugger shows you values.** You still do the working out.
