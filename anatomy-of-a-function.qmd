...
Last session you wrote two functions without me telling you what any of the pieces were called. This session is about naming those pieces, because once you can name them you can look them up, ask about them, and read the help page when something goes wrong. Then we start doing something with all that data you read in.
...
**One function.** Two completely different plots, and the only thing that changed is which column goes on the y axis.

That is harder than it looks, and if you have ever tried to write a function around `dplyr` or `ggplot2` you have probably hit the wall it runs into. The thing that gets through the wall is two curly braces, and we get to those at the end of the hour.
...
![The same parts, drawn out. This figure uses a different function as its example, and the pieces are the same ones.](figs/function-anatomy.png){fig-alt="A function definition with six parts highlighted in colour: the name, the function keyword, an argument, a default value, the body, and the return value." width=100%}
...
Let's plot something, because that is where defaults earn their keep.
...
Two lines, and ggplot2 is a pleasure to write, so wrapping that in a function would not buy much. "Make everything a function" is not the advice.
...
```{r}
#| label: default-state
plot_study_by_age <- function(data, state = unique(data$state_territory)) {
  ggplot(data, aes(x = prop_studying, y = age_group)) +
    geom_col(fill = "#1B9E77") +
    scale_x_continuous(labels = scales::percent) +
    labs(
...
The default here is worked out from the data itself, which R is happy to do.
...
The subtitle says `r unique(tas_2023$state_territory)` and I never typed it, because the data already knew.

And a default is only a default, so you can still say something else when you want to.
...
And you can mix the two, which is what most people actually do:
...
**Name the insides too.** Look back at `pct_studying()`. It says `studying <- sum(...)` and `people <- sum(...)`, so the division reads as studying over people. That was a choice, and it took about three seconds.
