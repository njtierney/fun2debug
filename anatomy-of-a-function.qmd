...
This chapter goes into a bit more depth on the naming of different parts of a function, and the other bits and pieces that go with how it is created. 

Once we know the names of the particular parts of functions, and the terminology in functions, it will set you up well to look them up, ask about it, and read the help page.
...
Today we are focussing on these two plots. Let's look at them, and the code underneath them.
...
This is just **one function**, with two pretty different plots! The thing that changed is what variable goes on the y axis.
...
ggplot2 expressed itself well, it is a pleasure to write, so wrapping that in a function would not buy much. **"Make everything a function"** is not always the answer. Like, I'm not sure if we would get out of a function like so:
...
- The x axis should be a percentage
...
plot_study_titled <- function(data,
                              state,
...
3. Which argument of `pct_studying()` would you never give a default to?
...
A default makes the common case easy. It will just as happily make a wrong case
quiet.

For step 3, `data`. A default for it would mean the function had an opinion
about whose data you meant.
...
I think the braces are a deliberate echo of `glue()`.
...
median_by <- function(data, group) {
  data |>
    group_by({{ group }}) |>
    summarise(median_prop = round(median(prop_studying), 3), .groups = "drop")
}
